...
bl_info = {
    "name": "UPBGE Rich Presence",
    "description": "Discord Rich Presence for UPBGE",
...
def connectRPC():
    try:
        rpcClient.connect()
        print("[UPBGERPC] Connected!")
        return True
    except ConnectionRefusedError:
        print("[UPBGERPC] Unable to connect: ConnectionRefusedError")
        return False
    except (FileNotFoundError, AttributeError, exceptions.InvalidPipe, AssertionError):
        print("[UPBGERPC] Unable to connect: Discord client not detected")
        return False
        
# RPC
rpcClient = Presence("1161042451990073394")
...
class blendPresence(bpy.types.AddonPreferences):
    ...
    displayVersion: bpy.props.BoolProperty(
        name = "UPBGE Version",
        description = "Displays UPBGE version",
        default = True,
    )
